# -*- coding: utf-8 -*-
"""
Created on Mon Jun 28 20:15:32 2021
...
Actual problem: Start at: http://py4e-data.dr-chuck.net/known_by_Raunaq.html
...
# position=18 ; count=7 ; http://py4e-data.dr-chuck.net/known_by_Raunaq.html
...
while (iter  <= count) and (len(tags) > position):     
    print('Retrieving: ', url)
    url =  tags[position -1].get('href', None)
    content = tags[position -1].contents[0]
    html = urlopen(url, context=ctx).read()
    soup = BeautifulSoup(html, "html.parser") 
    tags = soup('a')     
    if iter + 2  == count :        
